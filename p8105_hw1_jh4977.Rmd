...
We should know the data in this dataset:
```{r}
...
na.rm = TRUE), 1)` °F, with values ranging from 
`r round(min(early_january_weather$temp, na.rm = TRUE), 1)` °F to 
`r round(max(early_january_weather$temp, na.rm = TRUE), 1)` °F. The 
